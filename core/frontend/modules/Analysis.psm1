#
# Module: Analysis.psm1 v28.2.0 (Audit Sprint — M02/M04/M06 Hardened)
#
...
function Assert-ArchiveIntegrity {
    ...
    # ISSUE-002 FIX: Constant-time comparison prevents timing oracle attacks on hash values.
    $expectedBytes = [byte[]]::new($expectedHash.Length / 2)
    $actualBytes   = [byte[]]::new($actualHash.Length / 2)
    for ($i = 0; $i -lt $expectedBytes.Length; $i++) {
        $expectedBytes[$i] = [System.Convert]::ToByte($expectedHash.Substring($i * 2, 2), 16)
        $actualBytes[$i]   = [System.Convert]::ToByte($actualHash.Substring($i * 2, 2), 16)
    }
    $diff = 0
    for ($i = 0; $i -lt $expectedBytes.Length; $i++) {
        $diff = $diff -bor ($expectedBytes[$i] -bxor $actualBytes[$i])
    }
    if ($diff -ne 0) {
...
function Invoke-LogAnalysis {
    ...
    try {
        try {
            # Zip Bomb protection: reject archives whose uncompressed content exceeds 500MB.
            $maxUncompressedBytes = 500MB
            $zipFile = [System.IO.Compression.ZipFile]::OpenRead($ArchivePath)
            try {
                $totalUncompressed = ($zipFile.Entries | Measure-Object -Property Length -Sum).Sum
                if ($totalUncompressed -gt $maxUncompressedBytes) {
                    throw "SECURITY VIOLATION: Archive uncompressed size ($([math]::Round($totalUncompressed / 1MB, 1)) MB) exceeds safety limit ($([math]::Round($maxUncompressedBytes / 1MB)) MB). Possible Zip Bomb."
                }
            }
            ...
            Write-Status -Level ERROR -Message "Failed to extract '$ArchivePath'. $($_.Exception.Message)"
            return $null
        }
        ...
        # ISSUE-003 FIX: Escape ALL keyword sources to prevent ReDoS — not just CLI input.
        $keywords = if ($PSBoundParameters.ContainsKey('Keyword')) {
            @([regex]::Escape($Keyword))
        }
        else {
            $Configuration.AnalysisConfig.KeywordsToFlag | ForEach-Object { [regex]::Escape($_) }
        }
